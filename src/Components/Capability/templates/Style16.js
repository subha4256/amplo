        ...
        return(
            <>
              ...
              { this.props.isEditableProcess && <div style={{display:"block",position:"relative",zIndex:'9999'}}>
                ...
                <div className={"vrt-box vl-top "+ratingClass} style={{display:(this.props.i === 10)?"inline-block" : 'none'}}>
                    <div className={"box absolute1class1 class16-1 " + processClass + "box-white-3-style sm-txt vl-top mr-2"+ratingClass} style={{display:(this.props.i === 10)? 'inline-block' : 'none', border:`1px solid ${borderBlue}`}} onDragStart={!this.props.modelingMode ? (e) => this.props.handleGridDragStart(e, fname, pname, this.props.fname1,this.props.pname1, this.props.processData.ProcessLevel1Name, this.props.processData.DecompositionProcessLevel1ID, 10) : () => {}} draggable onDragOver={(e) => this.props.handleDragOver(e)} onDrop={(e) => this.props.handleDrop(e, fname,pname, this.props.fname1,this.props.pname1, this.props.processData.ProcessLevel1Name, this.props.processData.DecompositionProcessLevel1ID)}><span className="vtext vtext-temp3" style={{display: this.state['showLabel10'] === false ? 'none' : 'inline-block'}}>{this.props.processData ? <Link className="mt01" to={"/capability-modeling/"+this.props.templateId+"/"+this.props.processData.DecompositionProcessLevel1ID+"/"+this.props.functionId+"/"+this.props.phaseId+"/"+this.props.modelingMode+"/"+this.props.defaultVersion} title={this.props.processData.ProcessLevel1Name + '\n' + Rank}>{AggrScore+" "+this.props.processData.ProcessLevel1Name}</Link> : 'Add Process'}</span>
                    {this.props.isEditableProcess &&<div style={{position: 'relative',display:"block",position:"relative",zIndex:'9999'}}>
                    <span className="fa fa-edit float-left" onClick={()=>this.props.selectProcessLevelBox(this.props.processData.DecompositionProcessLevel1ID,'edit')} style={{ cursor: 'pointer'}} ></span><span className="fa fa-times text-danger float-right" onClick={()=>this.props.selectProcessLevelBox(this.props.processData.DecompositionProcessLevel1ID,'delete')} style={{ cursor: 'pointer' ,position: "absolute" , top: "208px" , right: "12px"}} ></span>
                    </div>}
                     </div>
