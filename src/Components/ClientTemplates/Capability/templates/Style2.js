...
class Style2 extends Component {
    ...
    handleTitleChange(e,index){
        ...
        this.props.handleTitleChange(e, 'style2', index, this.props.functionId, this.props.phaseId);
    }
    ...
    render() {
        ...
        let class0 = "box box-white sm-txt w-100 h-90 twotextboxwidth";
        if(this.props.processData[0]) {
            if(this.props.processData[0].HasDecomposition === "1") {
                class0 = "box bg-average sm-txt w-100 h-90 twotextboxwidth";
            }
        }
        let class1 = "box box-white sm-txt w-100 h-56 twotextboxwidth";
        if(this.props.processData[1]) {
            if(this.props.processData[1].HasDecomposition === "1") {
                class1 = "box bg-average sm-txt w-100 h-56 twotextboxwidth";
            }
        }
        let class2 = "box box-white sm-txt w-100 h-56 twotextboxwidth";
        if(this.props.processData[2]) {
            if(this.props.processData[2].HasDecomposition === "1") {
                class2 = "box bg-average sm-txt w-100 h-56 twotextboxwidth";
            }
        }
        let class3 = "box box-white two-box-custom sm-txt";
        if(this.props.processData[3]) {
            if(this.props.processData[3].HasDecomposition === "1") {
                class3 = "box bg-average two-box-custom sm-txt";
            }
        }
        return(
            <div className="style2">
                <div className="d-flex pstyle2">
                    <div className="mr-2">
                    ...
                    <div className="vrt-box">
                        <div className={class3}><span className="vtext spanforlink" title={this.state.processTitle3} style={{display: this.state['showLabel3'] === false ? 'none' : 'block'}}>{this.props.processData[3] ? <Link className="lineclamp2" to={"/client-capability-modeling/"+this.props.templateId+"/"+this.props.processData[3].processId+"/"+this.props.functionId+"/"+this.props.phaseId} title={this.props.processData[3].processTitle}>{this.props.processData[3].processTitle}</Link> : this.state.processTitle3}</span> <span className="fa fa-edit float-left fl-sl3" onClick={this.handleEditProcess.bind(this, 3)} style={{display: this.state['showLabel3'] === false ? 'none' : 'block'}}></span><span className="fa fa-times text-danger float-right" onClick={this.props.handleDeleteProcess.bind(this, this.props.processData[3])} style={{display: this.state['showLabel3'] === false ? 'none' : 'block'}}></span><input type="text" className="rotate-textbox-style2" style={{display: this.state['showLabel3'] === false ? 'block' : 'none'}} onBlur={() => this.handleTitleBlur(3, this.props.processData[3].processId)} onChange={(e) => this.handleTitleChange(e,3)} value={this.props.processData[3] ? this.props.processData[3].processTitle : 'Add Process'} /></div>
                    </div>
                    </div>
                </div>
            </div>
