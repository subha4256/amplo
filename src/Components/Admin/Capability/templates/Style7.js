    ...
    render() {
        ...
        return(
            <div className="style7">
                ...
                <div className={class7}><span className="spanforlink" title={this.state.processTitle7}  style={{display: this.state['showLabel7'] === false ? 'none' : 'block'}}>{this.props.processData[7] ? <Link to={"/capability-modeling/"+this.props.templateId+"/"+this.props.processData[7].processId+"/"+this.props.functionId+"/"+this.props.phaseId} title={this.props.processData[7].processTitle}>{this.props.processData[7].processTitle}</Link> : this.state.processTitle7}</span> <span className="fa fa-edit float-left" onClick={this.handleEditProcess.bind(this, 7)} style={{display: this.state['showLabel7'] === false ? 'none' : 'block'}}></span><span className="fa fa-times text-danger float-right" onClick={this.props.handleDeleteProcess.bind(this, this.props.processData[7])} style={{display: this.state['showLabel7'] === false ? 'none' : 'block'}}></span><input type="text" style={{display: this.state['showLabel7'] === false ? 'block' : 'none',width:"100%"}} onBlur={() => this.handleTitleBlur(7, this.props.processData[7].processId)} onChange={(e) => this.handleTitleChange(e,7)} value={this.props.processData[7] ? this.props.processData[7].processTitle : 'Add Process'} /></div>
            </div>
        )
    }
}
