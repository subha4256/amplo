...
class Style3 extends Component {
    ...
    handleTitleChange(e,index){
        ...
        this.props.handleTitleChange(e, 'style3', index, this.props.functionId, this.props.phaseId);
    }
    ...
    render() {
        ...
        let class2 = "box box-white sm-txt w-100 h-41s3 mb3 w-150px";
        if(this.props.processData[2]) {
            if(this.props.processData[2].HasDecomposition === "1") {
                class2 = "box bg-average sm-txt w-100 h-41s3 mb3 w-150px";
            }
        }
        let class3 = "box box-white sm-txt w-100 h-68s3 mb3 w-150px";
        if(this.props.processData[3]) {
            if(this.props.processData[3].HasDecomposition === "1") {
                class3 = "box bg-average sm-txt w-100 h-68s3 mb3 w-150px";
            }
        }
        ...
        let class5 = "box box-white box-white-3 sm-txt mr-2";
        if(this.props.processData[5]) {
            if(this.props.processData[5].HasDecomposition === "1") {
                class5 = "box bg-average box-white-3 sm-txt mr-2";
            }
        }
        let class6 = "box box-white box-white-3 sm-txt";
        if(this.props.processData[6]) {
            if(this.props.processData[6].HasDecomposition === "1") {
                class6 = "box bg-average box-white-3 sm-txt";
            }
        }
        return(
            <div className="style3">
                <div className="d-flex pstyle3">
                    <div className="mr-2">
                    ...
                    <div>
                        <div className="vrt-box d-flex">
                            <div className={class5}><span className="vtext vtext-temp3 spanforlink" title={this.state.processTitle5} style={{display: this.state['showLabel5'] === false ? 'none' : 'block'}}>{this.props.processData[5] ? <Link to={"/capability-modeling/"+this.props.templateId+"/"+this.props.processData[5].processId+"/"+this.props.functionId+"/"+this.props.phaseId} title={this.props.processData[5].processTitle}>{this.props.processData[5].processTitle}</Link> : this.state.processTitle5}</span> <span className="fa fa-edit float-left fl-sl3" onClick={this.handleEditProcess.bind(this, 5)} style={{display: this.state['showLabel5'] === false ? 'none' : 'block'}}></span><span className="fa fa-times text-danger float-right" onClick={this.props.handleDeleteProcess.bind(this, this.props.processData[5])} style={{display: this.state['showLabel5'] === false ? 'none' : 'block'}}></span><input type="text" className="rotate-textbox-style3" style={{display: this.state['showLabel5'] === false ? 'block' : 'none'}} onBlur={() => this.handleTitleBlur(5, this.props.processData[5].processId)} onChange={(e) => this.handleTitleChange(e,5)} value={this.props.processData[5] ? this.props.processData[5].processTitle : 'Add Process'} /></div>
                            <div className={class6}><span className="vtext vtext-temp3 spanforlink" title={this.state.processTitle6} style={{display: this.state['showLabel6'] === false ? 'none' : 'block'}}>{this.props.processData[6] ? <Link to={"/capability-modeling/"+this.props.templateId+"/"+this.props.processData[6].processId+"/"+this.props.functionId+"/"+this.props.phaseId} title={this.props.processData[6].processTitle}>{this.props.processData[6].processTitle}</Link> : this.state.processTitle6}</span> <span className="fa fa-edit float-left fl-sl3" onClick={this.handleEditProcess.bind(this, 6)} style={{display: this.state['showLabel6'] === false ? 'none' : 'block'}}></span><span className="fa fa-times text-danger float-right" onClick={this.props.handleDeleteProcess.bind(this, this.props.processData[6])} style={{display: this.state['showLabel6'] === false ? 'none' : 'block'}}></span><input type="text" className="rotate-textbox-style3" style={{display: this.state['showLabel6'] === false ? 'block' : 'none'}} onBlur={() => this.handleTitleBlur(6, this.props.processData[6].processId)} onChange={(e) => this.handleTitleChange(e,6)} value={this.props.processData[6] ? this.props.processData[6].processTitle : 'Add Process'} /></div>
                        </div>
                    </div>
                </div>
            </div>
